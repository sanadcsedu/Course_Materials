...
class MLP(object):
    ...
    def plot(self, x_axis, train_accuracy, test_accuracy, x_label, xstep = 1.0, file = None):
        ...
        plt.ylabel('Accuracy')
        plt.xlabel(x_label)
        plt.title('Accuracy vs Number Epochs [150 Hidden Units]')
        plt.legend(loc='best')
        if file is not None:
...
if __name__ == '__main__':
    ...
    #num_batches = [250, 500, 1000, 2000, 2500]  
    #learning_rate = [1e-1, 5e-2, 3e-2, 2e-2, 1e-2, 7e-3, 5e-3, 3e-3, 1e-3, 1e-4]
    num_batches = [1000]
    learning_rate = [7e-3]
    training_acc = []
    test_acc = []
    ...
    for ne in range(len(num_epochs)):
        ...
        for hu in range(len(learning_rate)):
            for nb in range(len(num_batches)):
                ...
                # test_acc = []
                # x_axis = []
                MiniBatchSize = int(int(train_x.shape[0]) / int(num_batches[nb]))
                ...
                #INSERT YOUR CODE FOR EACH EPOCH HERE
                    ...
                    for b in range(num_batches[nb]):
                        ...
                        total_loss += mlp.train(train_x[b * MiniBatchSize: (b+1) * MiniBatchSize, :], train_y[b * MiniBatchSize: (b+1) * MiniBatchSize, :], learning_rate[hu], 0.8, 0.001)
    ...
    #print(training_acc)
    #print(test_acc)
    #mlp.plot(x_axis, training_acc, test_acc, 'Number of Epochs', 10.0, 'AccuracyVsEpochs101.jpg')