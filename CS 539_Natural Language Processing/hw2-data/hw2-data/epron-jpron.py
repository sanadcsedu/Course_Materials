    ...
    def print_prob_WO(self, prob):
        ...
        for A in sorted(self.dict_freq.keys()):
            ...
            for B in sorted(self.dict_freq[A].keys()):
                ...
                if len(blist) > 1:
                    new_state = A + blist[0]
                    print("(1 ({} {} {} {}))".format(new_state, A, blist[0], self.dict_freq[A][B] / sum))
                    prev = new_state
                    for j in range(1, len(blist) - 1):
                        new_state += blist[j]
                        print("({} ({} *e* {}))".format(prev, new_state, blist[j]))

                    print("({} (1 *e* {}))".format(prev, blist[len(blist) - 1]))

                else:
                    print("(1 (1 {} {} {}))".format(A, blist[0], self.dict_freq[A][B]/sum))
