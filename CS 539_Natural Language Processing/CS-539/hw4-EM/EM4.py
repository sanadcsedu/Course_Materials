...
class EM:
    def __init__(self):
        ...
        self.all_jphons = defaultdict(int)
        self.all_ephons = defaultdict(int)
    ...
    def init2(self):
        _len = 0
        for i in range(len(self.all_ephons)):
            _len += len(self.all_jphons)

        for i in self.all_ephons:
            for j in self.all_jphons:
                self.p_ep_jp[i][j] = 1 / _len
