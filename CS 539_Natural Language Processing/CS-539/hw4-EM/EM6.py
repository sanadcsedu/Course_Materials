    ...
    def iterations(self, iter):
        ...
        for example in self.all_examples:
                ...
                for e_last in forward[i]:
                    for e_next in CHARACTERS:
                        try:
                            ...
                            score = math.log(forward[i][e_last]) + math.log(self.language_model[e_last][e_next]) + math.log(self.emission_model[e_next][emission])
                        except:
                            pdb.set_trace()
            ...
            for e_last in forward[n]:
                stop = '</s>'
                try:
                    score = math.log(forward[n][e_last]) + math.log(self.language_model[e_last][stop])
                except:
                    pdb.set_trace()
            ...
            for i, emission in reversed(list(enumerate(example))):
                for e_next in backward[i+2]:
                    ...
                    # print(f'  j = {j}')
                        try:
                            score = math.log(backward[i+2][e_next]) + math.log(self.language_model[e_last][e_next]) + math.log(self.emission_model[e_last][emission])
                        except:
                            pdb.set_trace()
            ...
            for e_last in backward[1]:
                start = '<s>'
                try:
                    score = math.log(backward[1][e_last]) * math.log(self.language_model[start][e_last])
                except:
                    pdb.set_trace()
