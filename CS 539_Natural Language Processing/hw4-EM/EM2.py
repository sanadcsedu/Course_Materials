        ...
        for epron in self.word_alignments:
            ...
            for i in range(0, n):
                ...
                for j in forward[i]:
                    ...
                    for k in range(1, min(m-j, 3)+1):
                        ...
                        #print(f'    take {jseg} -> forward[{i+1}][{j+k}] = {forward[i+1][j+k]}')
            ...
            for i in range(n, 0, -1):
                ...
                for j in backward[i]:
                    ...
                    for k in range(1, min(j, 3)+1):
                        ...
                        #print(f'    take {jseg} -> backward[{i-1}][{j-k}] = {backward[i-1][j-k]}')
            ...
            for z, alignment_z in enumerate(self.word_alignments[epron]):
                ...
                for u in range(len(eprons)):
                    ...
                    #print(f'    self.fractional_count[{epron}][{z}] += forward[{u}][{last_j}]({forward[u][last_j]}) * backward[{u+1}][{next_j}]({backward[u+1][next_j]}) * self.p_ep_jp[{epho}][{jseg}]({self.p_ep_jp[epho][jseg]}) / forward[{n}][{m}]({forward[n][m]})')
                    self.fractional_count[epron][z] += (forward[u][last_j] * backward[u+1][next_j] * self.p_ep_jp[epho][jseg]) / forward[n][m]
                    last_j = next_j
